// View: Land/Loteamentos — espelho V7 linhas 711–874
// Implementação completa em DOM puro (sem framework) usando host.querySelector
...
const DONUT_LABELS = ['Parque','Jardim Europa','Horizon Hills'];
const DONUT_SERIES = [80,72,48];
const VENDA_MEDIA_LOTE = 285437;
const INFRA_CONCLUIDA = 64.3;
const ESTOQUE_LOTES = 73;
...
<div class="col-span-12 lg:col-span-9 grid grid-cols-1 lg:grid-cols-3 gap-6">
...
export function mount(host, ctx) {
  ...
  // Drawer open/close (espelho V7: remove hidden + add open para animar)
  const drawer = host.querySelector('#drawer-loteamento');
  // O donutCard deve ser o card container (parent do chart), não o div interno do ApexCharts
  // que captura clicks no SVG. Vou usar o card container com data-drawer-trigger.
  ...
  if (donutCard && drawer) donutCard.addEventListener('click', (e) => {
    // Não abre drawer se o click foi no botão fullscreen
  ...
  if (drawerClose && drawer) drawerClose.addEventListener('click', () => { drawer.classList.remove('open'); drawer.classList.add('hidden'); });
    ...
    // Tabs (espelho V7: border-b-2 border-surface-tint = ativo, border-transparent = inativo)
    ...
    // Row expand (espelho V7 Borg.toggleRowExpand)
    ...
    // Cross-filter clear
    const clearBtn = host.querySelector('[data-cross-filter-clear]');
    if (clearBtn) clearBtn.addEventListener('click', () => showToast('Filtro limpo'));

  // Fullscreen: delegado ao handler global [data-fullscreen] em main.js → openChartFullscreen
