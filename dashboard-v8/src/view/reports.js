// View: Relatórios. Espelho V7 — 6 cards + modal popup com preview + download HTML + Abrir PDF.
// Arquitetura V8: módulo isolado, usa modal.js existente, lifecycle limpo.

import { initReveal } from '../ui/reveal.js';
import { openContentFullscreen, closeModal } from '../ui/modal.js';
import { escape, showToast } from './shared.js';
import { BRANDING_DEFAULTS, REPORTS, REPORT_SUMMARIES, VIEW_LABELS } from '../model/branding.js';

// Dados estáticos para manter parity com V7 (agora vindos de branding.js)
/* REPORTS moved to branding.js */
...
// SUMMARIES moved to branding.js (REPORT_SUMMARIES)
...
function _buildReportHTML(type) {
  ...
  // Fallbacks derivam de SETTINGS_DEFAULTS (única fonte de verdade — state/settings.js).
  // Antes (auditoria 2026-04-29): strings hardcoded duplicavam DEFAULTS e viravam zumbis se DEFAULTS mudasse.
...
export function mount(host, ctx) {
  function wireEvents() {
    ...
    host.querySelectorAll('button[data-recent]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (matchedType) generateReport(matchedType[1]); else _downloadReportHTML('executive');
      });
    });
  }
  ...
  // Read settings do store (ctx.store.get().settings)
