// Fragmented helpers and constants for Settings view
// Exports used by src/view/settings.js and keeps behavior identical

import { escape, showToast } from './shared.js';
import { isDarkMode, toggleDarkMode } from '../ui/dark-mode.js';

// Reusable visibility groups (split from large settings.js template)
...
// Lightweight CSS class used by inputs inside template
...
// Desktop-friendly, segment-based template generation moved here
...
// Helper to restore visibility toggles from settings into DOM
  ...
  if (settings.visibility) {
    try {
      ...
      host.querySelectorAll('#visibility-toggles input[type="checkbox"]').forEach(cb => {
        if (Object.prototype.hasOwnProperty.call(vis, cb.dataset.toggle)) {
          cb.checked = vis[cb.dataset.toggle];
        }
      });
    } catch {}
  }
...
// Persist visibility toggles to store
export function saveVisibility(host, { pushToStore, autosave }) {
  const visibility = {};
  host.querySelectorAll('#visibility-toggles input[type="checkbox"]').forEach(cb => {
...
// Wire events for the settings view (inputs, visibility, dark toggle, save)
export function wireSettingsEvents(host, { pushToStore, applyAnimationsPref, autosave, store }) {
  ...
  host.querySelectorAll('#visibility-toggles input[type="checkbox"]').forEach((cb) => {
    cb.addEventListener('change', () => saveVisibility(host, { pushToStore, autosave }));
  });
...
// Initialize reveal-on-scroll for elements with 'reveal' class
export function initReveal(host) {
  try {
